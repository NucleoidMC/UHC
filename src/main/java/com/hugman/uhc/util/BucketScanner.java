...
public final class BucketScanner {
	...
	private BucketScanner(Builder builder, BlockPos origin) {
		...
		ends.push(origin.asLong());
		endDepthMap.put(origin.asLong(), builder.maxDepth);
	...
	private void scan(long previousLong, int offsetX, int offsetY, int offsetZ) {
		...
		if(test(previousPos, mutable, set, ends)) {
			long offsetLong = mutable.asLong();
			int depth = endDepthMap.get(previousLong) - 1;
			if(depth > 0) {
				ends.push(offsetLong);
				endDepthMap.put(offsetLong, depth);
			}
			else {
				set.add(offsetLong);
			}
