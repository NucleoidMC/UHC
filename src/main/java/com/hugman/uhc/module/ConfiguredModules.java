...
public class ConfiguredModules {
	private static final TinyRegistry<ConfiguredModule> REGISTRY = TinyRegistry.newStable();
	...
	public static void register() {
		...
		serverData.registerReloadListener(new SimpleSynchronousResourceReloadListener() {
			...
			public Identifier getFabricId() {
				return new Identifier(Plasmid.ID, "configured_modules");
			}
			...
			public void apply(ResourceManager manager) {
				...
				Collection<Identifier> resources = manager.findResources("configured_modules", path -> path.endsWith(".json"));
				...
				for (Identifier path : resources) {
					try {
						...
						try (Reader reader = new BufferedReader(new InputStreamReader(resource.getInputStream()))) {
							...
							DataResult<ConfiguredModule> result = ConfiguredModule.CODEC.decode(JsonOps.INSTANCE, json).map(Pair::getFirst);
	...
	private static Identifier identifierFromPath(Identifier location) {
		...
		path = path.substring("configured_modules/".length(), path.length() - ".json".length());
		return new Identifier(location.getNamespace(), path);
	}

	@Nullable
	public static ConfiguredModule get(Identifier identifier) {
